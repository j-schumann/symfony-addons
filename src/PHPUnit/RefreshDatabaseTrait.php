 ...
 *  all tables, may require Vrok\DoctrineAddons\DBAL\Platforms\PostgreSQLTestPlatform
 *  to cascade the purge.
 *  On MySQL/MariaDB and SQLServer the tables are emptied with DELETE instead of
 *  TRUNCATE, see DB_PURGE_MODE. Emptying a table does not reset its identity
 *  generator on every platform, so this is done separately on all of them,
 *  see resetIdentities()
 *
 * "dropSchema" will drop all tables (and indices) and recreate them before each
 * test, use this when a test requires a genuinely fresh schema.
 *
 * "dropDatabase" will drop the entire database and recreate it before each
 * test.
 *
 * Per booted kernel "purge" only empties the tables, where both others drop and
 * recreate the whole schema, so it is the one to use unless a test really needs
 * a fresh schema. See the README for measurements and for when the difference
 * matters.
 *
 * With the cleanup method "purge", the ENV DB_PURGE_MODE selects how the tables
 * are emptied on MySQL/MariaDB. The default is "delete": on InnoDB, TRUNCATE is
 * a DDL operation that drops and recreates the tablespace file, which is paid
 * once per table and per test and dominates the runtime of a database-heavy
 * test suite, while DELETE is DML and magnitudes cheaper. Set it to "truncate"
 * to restore the previous behavior, e.g. for tests that insert very large
 * datasets before the cleanup, as DELETE is O(rows) where TRUNCATE is O(1).
 * The setting has no effect on other platforms: SQLServer cannot TRUNCATE
 * tables that are referenced by a foreign key, PostgreSQL and SQLite have no
...
trait RefreshDatabaseTrait
{
    ...
    protected static function bootKernel(array $options = []): KernelInterface
    {
        ...
        switch ($cleanupMethod) {
            ...
            case 'purge':
                ...
                // does not help here.
                // In MySQL/MariaDB, TRUNCATE is a DDL operation: InnoDB drops
                // and recreates the tablespace file, for each table and each
                // test, which is far more expensive than emptying the tables
                // with DELETE (@see DB_PURGE_MODE above).
                // So we switch to simple delete on both platforms, but this
                ...
                // Emptying the tables does not necessarily reset their identity
                // generators, but tests may rely on the generated IDs (e.g.
                // when asserting on IRIs like /items/1), so we reset them
                // ourselves. Only a TRUNCATE on MySQL/MariaDB does it for us.
                // This has to happen here, after the purge and before the
                // fixtures are loaded: ALTER TABLE is DDL and triggers MySQLs
                ...
                // Restore the default: the checks were only disabled for the
                // purge, the fixtures and the test itself should run with the
                // same settings as the application, e.g. for tests that assert
                // that a foreign key violation occurs.
                if ($isMysql) {
    ...
    /**
     * Resets the identity generators of all tables that have one, so the
     * records created in the tests always receive the same IDs.
     *
    ...
    protected static function resetIdentities(
        ...
        if ($platform instanceof MySQLPlatform || $platform instanceof MariaDBPlatform) {
            foreach (array_keys($identityTables) as $table) {
                // the value is clamped to MAX(id) + 1, so this is a no-op for
                // a table that was excluded from the purge and still has rows
        ...
        if ($platform instanceof PostgreSQLPlatform) {
            // TRUNCATE keeps the sequences, only "TRUNCATE ... RESTART IDENTITY"
            // would reset them, @see Vrok\DoctrineAddons\DBAL\Platforms\PostgreSQLTestPlatform
            foreach ($identityTables as $table => $meta) {
                // is_called = false lets the next record use the value itself.
                // The table name is passed quoted, as pg_get_serial_sequence
                // treats its first argument as a (potentially qualified) name
                // and would lowercase an unquoted one.
        ...
        if ($platform instanceof SQLServerPlatform) {
            foreach (array_keys($identityTables) as $table) {
                // DBCC CHECKIDENT makes the *next* record use RESEED + 1, but
                // only for a table that was inserted into before: for a table
                // that was never used, the first record uses the reseed value
                // itself, which would be 0. So we only reseed tables that have
                // a current identity value.
        ...
        if ($platform instanceof SQLitePlatform) {
            // SQLite stores the counters of all AUTOINCREMENT columns in this
            // table, it only exists when at least one such column is used
    ...
    /**
     * Returns all tables that have an identity / auto-increment column, in the
     * order in which the mapping defines them.
     *
     * The list is derived from the ORM metadata and not from the database:
     * The trait creates the schema itself, from that same metadata
     * (@see updateSchema()), and the SchemaTool marks a column as
     * auto-increment exactly when the entity uses the IDENTITY generator for a
     * single-field identifier, so both are in sync by construction. We apply
     * the same filters the ORMPurger uses to build its list of tables.
     *
     * Tables that are excluded from the purge (via the ORMPurgers $excluded or
     * a schema assets filter) are not filtered out here, they are only reset
     * together with all others.
     *
     * The result is cached, but as this is a trait, the cache is per test class
     * using it and not per process.
     *
     * @return array<string, array{name: string, column: string}> the quoted
     *                                                            table name, mapped to the unquoted table name and the
     *                                                            unquoted name of the identity column
     */
    protected static function getIdentityTables(
        ...
        foreach ($em->getMetadataFactory()->getAllMetadata() as $meta) {
            ...
            // Only the root class of a hierarchy owns the identity column:
            // With SINGLE_TABLE the children share the table of the root, with
            // JOINED they have their own table, but its primary key is a
            // foreign key to the root and not auto-increment. The children
            // inherit generatorType = IDENTITY in both cases,
